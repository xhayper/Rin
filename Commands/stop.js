...
exports.run = (client, msg, args, options) => {
  ...
  if (
    ...
    (options.queue.get(msg.guild.id)[0].voiceConnection &&
      options.queue.get(msg.guild.id)[0].voiceConnection.channel.id !==
        msg.member.voiceChannel.connection.channel.id)
  ) {
    return msg.channel.send(error2);
  }
