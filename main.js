...
class Function {
  ...
  checkPerms(member) {
    ...
    if (
      (this.botOwnerList.length() === 1 &&
        this.botOwnerList[0] === member.id) ||
      (this.botOwnerList.length() > 1 && this.botOwnerList.includes(member.id))
    ) {
...
if (!fs.existsSync("./Events")) return fs.mkdirSync("./Events");
  ...
  glob("./Events/*.js", (err, res) => {
    ...
    res.forEach(e => {
      ...
      if (!event.run)
        return console.log(
          `[EventManager] Event "${eventName}" doesn't have main function, Unloading it.`
        );
      ...
      x++;
      console.log(
        `[EventManager] Event "${eventName}" has been loaded. (${x}/${eventAmount})`
      );
    });
  });
...
if (!fs.existsSync("./Commands")) return fs.mkdirSync("./Commands");
  ...
  glob("./Commands/*.js", (err, res) => {
    ...
    res.forEach(c => {
      ...
      if (!command.run)
        return console.log(
          `[CommandManager] Command "${commandName}" doesn't have main function, Unloading it.`
        );
      x++;
      let commandData = {
      ...
      if (command.config) {
        if (command.config.aliases)
          commandData.aliases = command.config.aliases;
        commandData.config = command.config;
      }
      commandList.set(commandName.toLowerCase(), commandData);
      console.log(
        `[CommandManager] Command "${commandName}" has been loaded. (${x}/${eventAmount})`
      );
    });
  });
